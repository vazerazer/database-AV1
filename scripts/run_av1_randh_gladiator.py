#!/usr/bin/env python3
"""
scripts/run_av1_randh_gladiator.py
Targeted Candidate-Group Reference Calibration Runner (Op 941A)
Measures empirical perceptual quality (VMAF, PSNR-Y, SSIM, BPP) for
RandH's second title: Gladiator (2000) [Extended Cut].
"""
...
GLADIATOR_ANCHOR = {
    ...
    "year": 2000,
    "edition": "Extended Cut UHD BluRay",
    "av1_group": "RandH",
    "av1_rel_path": "Gladiator.2000.Extended.Cut.2160p.BluRay.DV.HDR10.Encode.AV1.DTS.5.1-RandH/Gladiator.2000.Extended.Cut.2160p.BluRay.DV.HDR10.Encode.AV1.DTS.5.1-RandH.mkv",
    "ref_group": "hallowed",
    "ref_format": "2160p x265 HDR/DV",
    "ref_rel_path": "Gladiator.2000.Extended.Cut.REPACK.UHD.BluRay.2160p.DDP.7.1.DV.HDR.x265-hallowed/Gladiator.2000.Extended.Cut.REPACK.UHD.BluRay.2160p.DDP.7.1.DV.HDR.x265-hallowed.mkv",
    "scenes": [
        {"name": "Near-Black / Shadow-Detail", "desc": "Imperial tent candlelight & low-light shadows", "start": "00:26:00", "duration": 10, "is_optional": False},
        {"name": "High-Motion", "desc": "Germania forest battle flaming arrows, cavalry charge & sword clash", "start": "00:10:30", "duration": 10, "is_optional": False},
        {"name": "Texture / Fine-Detail", "desc": "Zucchabar desert market rough stone architecture & linen grain", "start": "00:46:00", "duration": 10, "is_optional": False},
        {"name": "Colosseum Chariot Battle & Dust Stress (Optional)", "desc": "Battle of Carthage arena dust cloud, heavy 35mm grain & rapid chariot motion", "start": "01:25:00", "duration": 10, "is_optional": True}
    ]
}
...
def main():
    print("================================================================================")
    print("OP 941A: TARGETED CANDIDATE-GROUP CALIBRATION (RANDH GLADIATOR)")
    print("================================================================================")

    tmp_dir = os.path.join(MEDIA_ROOT, "scratch", "calibration_op941a")
    os.makedirs(tmp_dir, exist_ok=True)

    av1_full = os.path.join(ALTMOUNT_M4K, GLADIATOR_ANCHOR["av1_rel_path"])
    ref_full = os.path.join(ALTMOUNT_M4K, GLADIATOR_ANCHOR["ref_rel_path"])
    ...
    output_data = {
        "title": GLADIATOR_ANCHOR["title"],
        "year": GLADIATOR_ANCHOR["year"],
        "edition": GLADIATOR_ANCHOR["edition"],
        "av1_group": GLADIATOR_ANCHOR["av1_group"],
        "av1_bitrate_mbps": av1_mbps,
        "ref_group": GLADIATOR_ANCHOR["ref_group"],
        "ref_format": GLADIATOR_ANCHOR["ref_format"],
    ...
    out_json = os.path.join(REPO_ROOT, "evidence", "av1_calibration_randh_gladiator_raw.json")
